...
namespace OpenSMOKE
{
	...
	void NonAdiabaticFlameletLibraryReader::GetSpeciesAvailable(std::vector<std::string>& list_of_species)
	{
		std::string tag;

		boost::filesystem::path file_name = folder_name_ / "Output_0_Hz_0_kJkg.xml";
		...
		// Number of species available
		{
			...
			if (indices_node != 0)
			{
				...
				list_of_species.resize(number_of_species);
			}
			else
		...
		// Read available species
		{
			...
			if (profiles_size_node != 0)
			{
				...
				for (unsigned int i = 0; i<list_of_species.size(); i++)
						values >> list_of_species[i];
			}
			else
